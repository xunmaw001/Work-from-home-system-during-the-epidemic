    ...
    */
    ...
    public R info(@PathVariable("id") Long id, HttpServletRequest request){
        ...
        if(yonghu !=null){
            ...
            //级联表 部门信息
            //级联表
            BumenEntity bumen = bumenService.selectById(yonghu.getBumenId());
            if(bumen != null){
            BeanUtils.copyProperties( bumen , view ,new String[]{ "id", "createTime", "insertTime", "updateTime"});//把级联的数据添加到view中,并排除id和创建时间字段,当前表的级联注册表
            view.setBumenId(bumen.getId());
            }
            //修改对应字典表字段
...
//                            yonghuEntity.setBumenId(Integer.valueOf(data.get(0)));   //部门 要改的
    ...
    public R getCurrYonghu(HttpServletRequest request){
        ...
        if(yonghu !=null){
            ...
            //级联表
                            BumenEntity bumen = bumenService.selectById(yonghu.getBumenId());
            if(bumen != null){
                BeanUtils.copyProperties( bumen , view ,new String[]{ "id", "createDate"});//把级联的数据添加到view中,并排除id和创建时间字段
                view.setBumenId(bumen.getId());
            }
            //修改对应字典表字段
            dictionaryService.dictionaryConvert(view, request);
